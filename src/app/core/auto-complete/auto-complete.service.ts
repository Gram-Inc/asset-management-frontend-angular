...
export class AutoCompleteService
{
   ...
   private _ITUsers: BehaviorSubject<IUser[]> = new BehaviorSubject<IUser[]>([]);
   ...
   get ITUsers()
   {
      return this._ITUsers.asObservable();
   }
   ...
   //Get Requesters for Ticket
   getITUsers(page: number = 1, limit: number = 10, searchText: string = "")
   {
      return this._httpClient
         .get<IDTO>(`${this._baseUrl}/users/paginate`, {
            params: {
         ...
         .pipe(
            map(x => (x.data as IUser[]).filter(y => y.role != 'level3')),
            tap((response: IUser[]) =>
            {
               this._ITUsers.next(response);
            })
         );
   }
}
